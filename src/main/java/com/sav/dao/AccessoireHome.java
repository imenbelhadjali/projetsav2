...
public class AccessoireHome {
	...
	public Accessoire findById(java.lang.Integer id) {
		...
		try {
			Accessoire instance = (Accessoire) sessionFactory.getCurrentSession()
					.get("com.stage.pfe.dao.Accessoire", id);
			if (instance == null) {
	...
	@SuppressWarnings("unchecked")
	public List<Accessoire> findByNom(String n) {//select * from accessoire where cond;
		Criteria crit = sessionFactory.getCurrentSession().createCriteria(
				Accessoire.class).add(Restrictions.eq("nom", n));
		return crit.list();
	}
