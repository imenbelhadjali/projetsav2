...
public class StatutHome {
	...
	public Statut merge(Statut detachedInstance) {
		...
		try {
			Statut result = (Statut) sessionFactory.getCurrentSession()
	...
	public Statut findById(java.lang.Integer id) {
		...
		try {
			Statut instance = (Statut) sessionFactory.getCurrentSession()
					.get("com.stage.pfe.dao.Statut", id);
			if (instance == null) {
	...
	public List<Statut> findByNom(String n) {//select * from statut where cond;
		Criteria crit = sessionFactory.getCurrentSession().createCriteria(
				Statut.class).add(Restrictions.eq("nom", n));
		return crit.list();
	}
